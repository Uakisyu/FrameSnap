...
color 2
title ¡ù FrameSnap ¡¤ By iTufke ¡ù

echo ==== FrameSnap By iTufke ====
echo .                           .
echo    **** Using FFmpeg  ****
echo .                           .
echo =============================
...
if not exist "%FFMPEG%" (
  echo [Tuf.Bat] Error, ffmpeg.exe not found.
  echo [Tuf.Bat] Place ffmpeg.exe in the current running directory.
  echo.
  pause
  exit /b 1
)
...
if "%videopath%"=="" (
  echo [Tuf.Bat] Error, the input is empty.
  echo.
  pause
  exit /b 1
)
...
if not exist "%videopath%" (
  echo [Tuf.Bat] Error, video not found: "%videopath%".
  pause
  exit /b 1
)
...
if "%fps%"=="" (
  echo.
  echo [Tuf.Bat] Error, the input is empty. Used default value is 5.
  set "fps=5"
)
...
for /f "delims=0123456789" %%A in ("%fps%") do (
  echo.
  echo [Tuf.Bat] Error, the input not a pure number. Used default value is 5.
  set "fps=5"
)

rem Display settings
echo.
echo [Tuf.Bat] Confirm the settings info:
echo.
echo Video: "%videopath%"
echo Output folder: "!outdir!"
echo Frame rate£º !fps! frame per second
echo.
echo [Tuf.Bat] Press Anykey to start FrameSnap.
echo.
pause
...
echo =============================
echo.
echo [Tuf.Bat] Complete, and all results have been saved to the "!outdir!" folder.
echo.
echo [Tuf.Bat] Press Anykey to exit.
pause
endlocal